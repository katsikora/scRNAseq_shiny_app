...
RUN yum install -y epel-release && \
    ...
    yum install -y openssl-devel curl libcurl-devel mesa-libGLU libpng-devel cairo-devel libxml2-devel xorg-x11-server-devel libX11-devel libXt-devel  && \
    mkdir /data && \
    mkdir /etc/automount && \
